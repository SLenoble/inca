comptage_balise <- function(nom_chemin,id_fic,nom_output) {
  ...
  liste_fic <- list.files(nom_chemin)
  ...
  liste_balise_hist <- read.csv2(paste0(chemin_output,nom_output,".csv"))  
  ...
  file.rename(from = paste0(chemin_output, nom_output,".csv"), to = paste0(chemin_output, nom_output,"_",format(Sys.time(),"%Y%m%d%H%M%S"),".csv"))
  
  ## on exporte la nouvelle
  write.table(res_tot_agg, paste0(chemin_output,nom_output,".csv"), row.names = FALSE, sep = ";")
}
